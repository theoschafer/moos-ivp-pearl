...
def plot_prediction(noise_x, noise_y, pred_x, pred_y):
    plt.figure(2)
    plt.plot(noise_x, noise_y, 'r+')
    #plt.plot(noise_x, noise_y, color='r', linewidth=4, label='pos_real')
    """ for i in range(len(pred_x)):
        plt.plot(pred_x[i], pred_y[i], color='g', alpha=0.6) """
    
    #plt.plot(pred_x[len(pred_x)-1], pred_y[len(pred_x)-1], color='g', alpha=0.6)
    plt.plot(pred_x[len(pred_x)-1], pred_y[len(pred_x)-1], 'bo')
    plt.legend()
...
def plot_all(cv, ca, ct, noise_x, noise_y, pred_x, pred_y):
    plt.figure(1)
